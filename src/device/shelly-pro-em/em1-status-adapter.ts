 ...
 *
 * Extracts active power metrics from Shelly Pro EM device status and converts
 * them to a standardized energy information structure.
 */
...
> {
   ...
   * @returns A promise that resolves to EnergyInformation with power data, or
   * undefined if power data is unavailable
   */
...
/**
 * Singleton adapter instance for converting `EM1Status` to
 * `EnergyInformation`.
 *
 ...
 *
 * Returns a singleton adapter instance that transforms EM1Status data into
 * EnergyInformation.
 *
 * Implements the AdapterFactory pattern without requiring configuration
 * options.
 */
...
> = {
   ...
   *
   * No configuration options are required as the adapter uses a fixed
