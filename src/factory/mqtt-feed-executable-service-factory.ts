 ...
 * Extracts the power value from EnergyInformation and passes it to the
 * underlying callback.
 *
 * @param callbackProperties - The callback properties expecting numeric power
 * values
...
/**
 * Factory for creating MQTT feed executable services with flexible
 * communication modes.
 *
 * Supports both pull (periodic polling) and push (event-driven) MQTT feed
 * strategies.
 *
 * Automatically routes to the appropriate service factory based on
 * configuration.
 */
export class MqttFeedExecutableServiceFactory implements ExecutableServiceFactory<MqttFeedExecutableServiceFactoryProperties> {
   ...
   *
   * @param options - Configuration options including energy type, device,
   * mode, and callbacks
